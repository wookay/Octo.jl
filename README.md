...
```julia
(v1.11) pkg> add Postgres # for PostgreSQL (depends on Postgres.jl 2.1)
(v1.11) pkg> add SQLite   # for SQLite     (depends on SQLite.jl 1.8)
(v1.11) pkg> add MySQL    # for MySQL      (depends on MySQL.jl 1.6)
(v1.11) pkg> add DuckDB   # for DuckDB     (depends on DuckDB.jl 1.5)
```
